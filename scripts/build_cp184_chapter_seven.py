#!/usr/bin/env python3
"""Render and concatenate the Chapter 7 preview in its approved lesson order."""
...
def resolve_scene(scenes_dir: Path, checkpoint: int, candidates: tuple[str, ...]) -> Path:
    ...
    # Fallback for an older checkpoint whose filename may differ slightly.
    checkpoint_tokens = {
        169: ("eigen", "value"),
        171: ("characteristic",),
        172: ("eigen", "comput"),
...
def main() -> int:
    ...
    if not title_file.exists():
        raise FileNotFoundError(title_file)
    ...
    concat_file = media_dir / "chapter_seven_concat.txt"
    concat_file.write_text(
        "".join(f"file '{p.as_posix()}'\n" for p in segments),
        encoding="utf-8",
    )
    output = output_dir / "ChapterSeven_EigenvaluesAndEigenvectors_preview.mp4"
    run(
        [
    ...
    print(f"\nChapter 7 preview assembled at:\n{output}")
    return 0
