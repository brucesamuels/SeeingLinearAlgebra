...
python -m pytest -q tests/test_cp167_chapter_six_hd.py
...
if (( ${#missing[@]} > 0 )); then
  print -u2 -- "CP167 HD assembly is missing approved scene sources:"
...
for command_name in python ffmpeg ffprobe; do
  if ! command -v "$command_name" >/dev/null 2>&1; then
    print -u2 -- "CP167 check requires '$command_name' on PATH."
    exit 1
  fi
done

print -- "CP167 checks passed: all 17 approved Chapter 6 scene sources and render tools are present."
