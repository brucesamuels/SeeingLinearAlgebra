...
if sys.version_info[:2] != (3, 12):
    raise SystemExit(f"CP224 requires Python 3.12; found {sys.version.split()[0]}")
PY
python -m manim --version | grep -q "v0.21.0" || {
  print -u2 -- "CP224 requires Manim Community v0.21.0."
  exit 1
}
for command_name in ffmpeg ffprobe; do
  command -v "$command_name" >/dev/null 2>&1 || {
    print -u2 -- "CP224 requires $command_name."
    exit 1
  }
done
...
print -- "Checkpoint 224 checks passed on Python 3.12 / Manim 0.21.0."
