...
MODULE_PATH = Path(__file__).parents[1] / "scripts" / "build_cp241_graph_chapter_preview.py"
SPEC = importlib.util.spec_from_file_location("graph_chapter_preview_builder", MODULE_PATH)
...
def test_assembly_contains_cp226_through_cp240_in_order():
    assert checkpoints() == list(range(226, 241))


def test_title_precedes_lessons_and_synthesis_closes_chapter():
    assert MODULE.CHAPTER_CLIPS[0][2] == "GraphsNetworksLaplacianTitleCard"
    assert MODULE.CHAPTER_CLIPS[-1][2] == "GraphChapterSynthesisPresentation"


def test_probability_sequence_preserves_requested_progression():
    classes = scene_classes()
    assert classes.index("GraphRandomWalkPresentation") < classes.index("MarkovLongTermBehaviorPresentation")
    assert classes.index("MarkovLongTermBehaviorPresentation") < classes.index("DirectedPageRankPresentation")
    assert classes.index("DirectedPageRankPresentation") < classes.index("GraphFundamentalSubspacesPresentation")
...
def test_resolve_scenes_reports_incomplete_chapter(tmp_path):
    ...
    with pytest.raises(FileNotFoundError, match="Missing Graphs, Networks"):
...
def test_builder_renders_fresh_validates_and_uses_stream_copy():
    ...
    assert "GraphsNetworksLaplacian_preview.mp4" in source
